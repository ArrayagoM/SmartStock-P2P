...
export default function App() {
  ...
  const updateAlerts = (currentInventory) => {
    ...
    currentInventory.forEach((item) => {
      if (item.vencimiento && item.vencimiento !== 'N/A') {
        ...
        if (diffDays < 0) currentAlerts.push({ ...item, status: 'vencido', days: Math.abs(diffDays) });
  ...
  const handlePointerDown = (e) => {
    ...
    setLastAction(null);
    setIsListening(true);
    try { recognitionRef.current.start(); } catch(e){} // Prevenir error si ya estaba escuchando
  };
  ...
  const processTextWithAI = async (text) => {
    ...
    // AQUÍ ES DONDE LLAMAMOS A LA VARIABLE DE ENTORNO EN LUGAR DE ESCRIBIR LA CLAVE
      ...
      <div className="flex-none bg-white p-6 rounded-b-[40px] shadow-sm border-b border-slate-200 flex flex-col items-center justify-center relative z-0">
        ...
        <div className="h-16 flex items-center justify-center w-full">
          {isListening ? (
            ...
            <div className={`px-5 py-2.5 rounded-full flex items-center gap-2 text-sm font-bold shadow-sm transition-all ${
      ...
      <main className="flex-1 overflow-y-auto p-4 bg-slate-50">
        ...
        {alerts.length > 0 && (
          <div className="mb-6">
            ...
            <div className="space-y-2">
              {alerts.slice(0, 3).map(alert => ( // Mostramos máximo las 3 alertas más urgentes
                <div key={`alert-${alert.id}`} className="bg-red-50 p-3 rounded-2xl border border-red-100 flex justify-between items-center">
                   <div>
                     <h3 className="font-bold text-slate-800 text-sm leading-tight">{alert.nombre}</h3>
        ...
        <div>
          ...
          {inventory.length === 0 ? (
            ...
            <div className="space-y-2">
              {inventory.slice(0, 10).map(item => ( // Mostramos solo los últimos 10 escaneos
                <div key={item.id} className="bg-white p-3 rounded-2xl shadow-sm border border-slate-100 flex justify-between items-center">
                   <div>
                     <h3 className="font-bold text-slate-700 text-sm">{item.nombre}</h3>
                     <div className="flex gap-2 text-xs text-slate-500 mt-0.5">
                       <span>L: {item.lote}</span>
                       <span>•</span>
                       <span className={alerts.some(a => a.id === item.id) ? 'text-red-600 font-bold' : 'text-green-600 font-bold'}>
                         V: {item.vencimiento}
                       </span>
                     </div>
                   </div>
